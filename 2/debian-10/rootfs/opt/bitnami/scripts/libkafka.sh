...
zookeeper_get_tls_config() {
    ...
    # check if certificates are in PEM or JKS formats
    if [[ -f "$KAFKA_CERTS_DIR"/zookeeper.truststore.pem ]]; then
        ext="pem"
    fi

    # Check if a keystore exists (not needed when zookeeper's clientAuth is disabled or optional)
    if [[ -f "$KAFKA_CERTS_DIR"/zookeeper.keystore."$ext" ]]; then
        keystore_opts="-Dzookeeper.ssl.keyStore.location=${KAFKA_CERTS_DIR}/zookeeper.keystore.${ext} \
